...
public class Lz77EncoderDecoder {
    ...
    private static int lzBitSetIndex = 0;
    ...
    public Lz77EncoderDecoder(int searchBufferSize, int windowSize) {
        ...
        for (int i = 0; i < 8; i++) {
            finalOutPut.set(lzBitSetIndex++, lengthBit.get(i));
        }
    }
     ...
     */
    public BitSet CompressLz(String fileToCompress, String outPutPath) {
     ...
     */
    public void deCompress(ArrayList<Byte> compressed, String outputDecompress) {
        ...
        try {
            ...
            //convert arrayList to bitset

            BitSet set = byteArrayToBitSet(compressed);

            //first 8 bits represent the length of the triplets
            int tripletSize = 0;
            while (lzBitSetIndex < 8) {
                if (set.get(lzBitSetIndex)) {
                    tripletSize += Math.pow(2, lzBitSetIndex);
                }
                lzBitSetIndex++;
            }


            while (lzBitSetIndex < set.length()) {
                //decode one triplet
                //char
                int bValue = 0;
                for (int i = 0; i < 8; i++) {
    ...
    public static BitSet singeByteToBitSet(byte b){

        // replace with input from stream
    ...
    public static BitSet byteArrayToBitSet(ArrayList<Byte> arr){

        BitSet finalBitSet = new BitSet(arr.size()*8);
        BitSet singelByte = new BitSet(8);

        for (int i = 0; i <arr.size() ; i++) {
            singelByte = singeByteToBitSet(arr.get(i));
            int temp = 0;
            for (int j = i*8; j <(i+1)*8 ; j++) {
                //copy 8 bits from singel to big array
                if(singelByte.get(temp)){
                    finalBitSet.set(j);
                }
                temp++;
            }
    ...
    private void encodeToBitSet(DictionaryItem item) {
        ...
        //for char
        for (int i = 0; i < 8; i++) {
            finalOutPut.set(lzBitSetIndex++, chBit.get(i));
        }
        ...
        //for char
        for (int i = 0; i < log2(searchBufferIndex); i++) {
            finalOutPut.set(lzBitSetIndex++, copyBit.get(i));
        }
    }
